...
dados_resp2$Internação = ifelse(dados_resp2$Internação == 1, "Sim",                   ifelse(dados_resp2$Internação == 2, "Não",                                        ifelse(dados_resp2$Internação == 9, "Ignorado","")))

dados_resp2$UTI = ifelse(dados_resp2$UTI==1,"Sim",ifelse(dados_resp2$UTI==2,"Não",ifelse(dados_resp2$UTI==9,"Ignorado","")))
...
dados_resp3$UTI = ifelse(dados_resp3$UTI==1,"Sim",ifelse(dados_resp3$UTI==2,"Não",ifelse(dados_resp3$UTI==9,"Ignorado","")))

dados_resp3$SUPORT_VEN =ifelse(dados_resp3$SUPORT_VEN==1,"Sim",ifelse(dados_resp3$SUPORT_VEN==2,"Não",ifelse(dados_resp3$SUPORT_VEN==9,"Ignorado",ifelse(dados_resp3$SUPORT_VEN=="","Ignorado",""))))
...
```{r}
covid_resp$UTI = ifelse(covid_resp$UTI==1,"Sim",ifelse(covid_resp$UTI==2,"Não",ifelse(covid_resp$UTI==9,"Ignorado","")))
ggplot(data= covid_resp,aes(x=Vacina, fill=UTI))+geom_bar(position = "fill")+ #position = fill da a proporção
  labs(y="Proporção")+theme_minimal()#?não muda muito a proporção de internação por quem tomou vacina, parece um pouco maior para quem não tomou
...
covid_resp$Internação = ifelse(covid_resp$Internação==1,"Sim",ifelse(covid_resp$Internação==2,"Não",ifelse(covid_resp$Internação==9,"Ignorado","")))

ggplot(data= covid_resp,aes(x=Vacina, fill=Internação))+geom_bar(position = "fill")+ #position = fill da a proporção
